...
//! over the C ABI, and this is the Rust one. It is what a provider writes
//! (ADR-0061): a [`ContractFactory`], and one line naming it to the SDK's
...
//! separate work under its provider's license.
//!
...
//! is the SDK's now, written once for every Rust contract.
...
mod tests {
    ...
    fn the_identity_contract_holds_any_stream_and_names_itself() {
        ...
        let stream = Stream::new(sdk::StreamId::new(1), vec![0xff, 0], None);
